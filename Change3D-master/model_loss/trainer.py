...
from model_loss.x3d import create_x3d
from model_loss.change_decoder import ChangeDecoder
from model_loss.caption_decoder import CaptionDecoder
from model_loss.utils import weight_init
...
class Encoder(nn.Module):
    ...
    def __init__(self, args: Any, embed_dims: List[int]) -> None:
        ...
        # Perception frames are deterministically initialized by interpolation
        # between pre/post images (instead of random learnable tokens).
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, output_final: bool=False) -> List[torch.Tensor]:
        ...
        """        
        # Build perception frames from processed inputs: t1' - t2'.
        # Here x/y are the two processed images fed into the network.
        perception_base = torch.abs(x - y).unsqueeze(2)  # [B, C, 1, H, W]

        if self.args.num_perception_frame > 0:
            percep_frames = perception_base.repeat(1, 1, self.args.num_perception_frame, 1, 1)
        else:
            percep_frames = x.new_zeros(x.size(0), x.size(1), 0, x.size(2), x.size(3))

        # Combine into sequence [input, perception..., target]
        frames = torch.cat([
            x.unsqueeze(2),
            percep_frames,
            y.unsqueeze(2)
...
class Trainer(nn.Module):
    ...
    def update_bcd(self, x: torch.Tensor, y: torch.Tensor, return_features: bool = False) -> torch.Tensor:
        """
        ...
        #sun
        if return_features:
            prediction, high_dim_feat = self.decoder(perception_change_feat, return_features=True)
            return prediction, high_dim_feat
        else:
