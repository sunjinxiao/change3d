...
class Trainer(nn.Module):
    ...
    def update_bcd(self, x: torch.Tensor, y: torch.Tensor, return_features: bool = False):
        """
        ...
        Args:
            x: Input frame tensor with shape [B, C, H, W]
            y: Target frame tensor with shape [B, C, H, W]
            return_features: Whether to return change features for auxiliary losses

        Returns:
            Predicted frame tensor, and optionally change features
        """
        ...
        if return_features:
            return prediction, perception_change_feat
